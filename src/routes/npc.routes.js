...
router.get("/npc", async (req, res) => {
    try {
        if (req.session.userId) {
            let campaigns = (
                await Campaign.distinct("_id", { players: req.session.userId })
            ).map((campaign) => campaign.toString());

            let campaignsDm = (
                await Campaign.distinct("_id", { dm: req.session.userId })
            ).map((campaign) => campaign.toString());
            ...
            res.status(200).json({
                payload: Array.from(new Set(npcs.map((a) => a.id))).map((id) =>
                    npcs.find((a) => a.id === id),
                ),
            });
        } else {
    ...
    try {
        if (req.session.userId) {
            Npc.findOneAndUpdate(
                { _id: req.body._id, createdBy: req.session.userId },
                req.body,
                function (err) {
                    if (err)
                        return res
